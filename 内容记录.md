...
#### 网格设置
可以使用
matplotlib.pyplot.grid()————来调整网格的状态
具体参数在书上
最后再加一个 
plt.show()————才能显示出我画出的图像
...
#### 图的大小 ，分辨率，保存
plt.figure(figsize=(10,8),dpi=300))——————设置大小，分辨率
...
#### 饼图 
使用 matplotlib.pyplot里面的pie()函数

表示占比的
#### 直方图 
将连续的数值型变量映射到x轴，y轴，还可以进行拟合。

#### 散点图

matplotlib.pyplot 的 scatter函数

可以使用seaborn的joinplot，可以自动绘制每个变量的边际分布

可以加一个kind='hex'，可以通过颜色深浅来显示区域内的散点数量

可以通过散点图矩阵，快速检验多个变量之间的相关性
使用 Seaborn的pairplot函数就可以了
...
#### 分面图形
axes函数可以在一张图里多填写几个坐标轴



plt.subplots可以构建多个坐标轴

可以进行适当的设置，让他们共享x轴或y轴，
调整sharex，sharey的参数即可



可以使用plt.subplot分别定义每个函数
fig=plt.figure(figsize=(10,10))——————先造个背景
